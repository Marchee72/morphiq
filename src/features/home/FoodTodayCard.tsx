        ...
        <>
          ...
          <div style={{ marginTop: 12, marginBottom: 8 }}>
            <div style={{ display: 'flex', height: 8, borderRadius: 999, overflow: 'hidden', background: 'var(--ui-outline)' }}>
              {pPct > 0 && <div style={{ width: `${pPct}%`, background: 'var(--ui-primary)' }} title={`Protein: ${totalP}g (${pPct}%)`} />}
              {cPct > 0 && <div style={{ width: `${cPct}%`, background: '#1A7F4B' }} title={`Carbs: ${totalC}g (${cPct}%)`} />}
              {fPct > 0 && <div style={{ width: `${fPct}%`, background: '#7C4DFF' }} title={`Fat: ${totalF}g (${fPct}%)`} />}
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 600, color: 'var(--ui-text-secondary)', marginTop: 6 }}>
              <span style={{ color: 'var(--ui-primary)' }}>Protein {totalP}g ({pPct}%)</span>
              <span style={{ color: '#1A7F4B' }}>Carbs {totalC}g ({cPct}%)</span>
              <span style={{ color: '#7C4DFF' }}>Fat {totalF}g ({fPct}%)</span>
            </div>
          </div>
